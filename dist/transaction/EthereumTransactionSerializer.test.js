...
test('Serialize a send tx', () => {
    ...
    const serializer = new EthereumTransactionSerializer_1.EthereumTransactionSerializer(1);
    ...
    const valid = serializer.verifySend(transaction, 'from', '0x8017877A1C06efbc7f444AC709119C1e209F26Ee', 'ETHEREUM:ETH', '12340000000000000');
    expect(valid).toBe(true);
    const nvalid = serializer.verifySend(transaction, 'from', '0x8017877A1C06efbc7f444AC709119C1e209F26Ee', 'ETHEREUM:ETH', '12340000000000001');
...
test('Serialize a smart contract tx', () => {
    ...
    expect(res.serializedTransaction).toBe(serializedTransaction);
    expect(res.signableHex).toBe(signableHex);
    const valid = serializer.verifySend(transaction, 'from', '0x8017877A1C06efbc7f444AC709119C1e209F26Ee', `ETHEREUM:${FRM}`, '112456001');
    expect(valid).toBe(true);
    const nvalid = serializer.verifySend(transaction, 'from', '0x8017877A1C06efbc7f444AC709119C1e209F26Ee', `ETHEREUM:${FRM}`, '112456002');
    expect(nvalid).toBe(false);
});
//# sourceMappingURL=EthereumTransactionSerializer.test.js.map