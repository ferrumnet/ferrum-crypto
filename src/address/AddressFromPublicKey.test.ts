...
test('ETH', () => {
    ...
    const actual = new AddressFromPublicKey().forNetwork('ETHEREUM', dummyPublicKeyCompressed, dummyPublicKey);
    expect(actual).toBe(expectedAddress);
});

test('FRM', () => {
    const expectedAddress = 'fx5d91e4167207740f73c1e08d62B2564beE979Ef3';
    const actual = new AddressFromPublicKey().forNetwork('FERRUM', dummyPublicKeyCompressed, dummyPublicKey);
    expect(actual).toBe(expectedAddress);
});

test('BNB', () => {
    const expectedAddress = 'bnb12cwfj4vv63pzwrty4j85rd74gkm8gdutnlrqcj';
    const actual = new AddressFromPublicKey().forNetwork('BINANCE', dummyPublicKeyCompressed, dummyPublicKey);
    expect(actual).toBe(expectedAddress);
});
